...
* New `test` argument (`"Chisq"` / `"SatorraBentler"`) to `penalized_longcfa()` enables experimental fit measures
...
* New `plavaan_args` list to `penalized_longcfa()` to forward extra options to `plavaan`, including penalty
